    ...
    public class InputInspectionMaterialService : IInputInspectionMaterialService
    {
        ...
        public async Task<int> Create(InputInspectionMaterialViewModel viewModel)
        {
            ...
            if (model == null)
            {
                ...
                model = new DyeingPrintingAreaInputModel(viewModel.Date, viewModel.Area, viewModel.Shift, bonNo, viewModel.Group, viewModel.InspectionMaterialProductionOrders.Select(s =>
                     ...
                     viewModel.Date, s.MaterialOrigin, s.ProductTextile.Id, s.ProductTextile.Code, s.ProductTextile.Name, s.ProductionOrder.CreatedUtc)).ToList());
            ...
            else
            {
                foreach (var item in viewModel.InspectionMaterialProductionOrders)
                {
                    var modelItem = new DyeingPrintingAreaInputProductionOrderModel(viewModel.Area, item.ProductionOrder.Id, item.ProductionOrder.No,
                        ...
                        item.InputQuantity, item.FinishWidth,viewModel.Date, item.MaterialOrigin, item.ProductTextile.Id, item.ProductTextile.Code, item.ProductTextile.Name, item.ProductionOrder.CreatedUtc);
        ...
        public ListResult<InputInspectionMaterialProductionOrderViewModel> ReadInputIM(string productionOrderId, string unit, DateTimeOffset? dateFrom, DateTimeOffset? dateTo, int page, int size, string order, int offset)
        {
           long POid = Convert.ToInt64(productionOrderId); 
            ...
            {
                query = query.Where(s => dateFrom.Value.Date <= s.DateIn.AddHours(offset).Date &&
                            s.DateIn.AddHours(offset).Date <= dateTo.Value.Date);
            }
            else if (!dateFrom.HasValue && dateTo.HasValue && productionOrderId==null && unit==null)
            {
                query = query.Where(s => s.DateIn.AddHours(offset).Date <= dateTo.Value.Date);
            }
            else if (dateFrom.HasValue && !dateTo.HasValue && productionOrderId==null && unit==null)
            {
                query = query.Where(s => dateFrom.Value.Date <= s.DateIn.AddHours(offset).Date);
            ...
            {
                query = query.Where(s => dateFrom.Value.Date <= s.DateIn.AddHours(offset).Date &&
                           s.DateIn.AddHours(offset).Date <= dateTo.Value.Date && s.ProductionOrderId == POid);
            }
            else if (!dateFrom.HasValue && !dateTo.HasValue && productionOrderId==null && unit!=null)
            {
            ...
            {
                query = query.Where(s => dateFrom.Value.Date <= s.DateIn.AddHours(offset).Date &&
                           s.DateIn.AddHours(offset).Date <= dateTo.Value.Date && s.Unit == unit);
            }
            else if (dateFrom.HasValue && dateTo.HasValue && productionOrderId!=null && unit!=null)
            {
                query = query.Where(s => dateFrom.Value.Date <= s.DateIn.AddHours(offset).Date &&
            ...
            //var abc = query.Where(x=>x.IsDeleted==true).ToList();
            var data = query.Skip((page - 1) * size).Take(size).Select(s => new InputInspectionMaterialProductionOrderViewModel()
            {
                Id = s.Id,
                Balance = s.Balance,
        ...
        public async Task<int> Update(int id, InputInspectionMaterialViewModel viewModel)
        {
            ...
            var model = new DyeingPrintingAreaInputModel(viewModel.Date, viewModel.Area, viewModel.Shift, viewModel.BonNo, viewModel.Group, viewModel.InspectionMaterialProductionOrders.Select(s =>
                 ...
                 s.FinishWidth,viewModel.Date, s.MaterialOrigin, s.ProductTextile.Id, s.ProductTextile.Code, s.ProductTextile.Name, s.ProductionOrder.CreatedUtc)

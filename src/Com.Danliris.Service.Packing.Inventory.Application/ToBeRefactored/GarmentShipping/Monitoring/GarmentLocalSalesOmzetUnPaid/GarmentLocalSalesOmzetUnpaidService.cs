...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.GarmentShipping.Monitoring.GarmentLocalSalesOmzetUnPaid
{
    public class GarmentLocalSalesOmzetUnPaidService : IGarmentLocalSalesOmzetUnPaidService
    {
...

            {
                ...
                foreach (KeyValuePair<string, List<GarmentLocalSalesOmzetUnPaidViewModel>> LSNumber in dataByLSNo)
                {
                    ...
                    foreach (GarmentLocalSalesOmzetUnPaidViewModel item in LSNumber.Value)
                    {
                        ...
                        string LSDate = item.LSDate == new DateTime(1970, 1, 1) ? "-" : item.LSDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
        ...
        //
        public IQueryable<GarmentDetailLocalSalesOmzetUnPaidViewModel> GetDataDetail(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            var query = repository.ReadAll();
            ...
            var newQ = (from a in query
                        ...
                        select new GarmentDetailLocalSalesOmzetUnPaidViewModel
                        {
                            LSNo = a.NoteNo,
                            LSDate = a.Date,
        ...
        public MemoryStream GenerateExcelDetail(DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            else
            {
                ...
                foreach (var d in Query)
                {
                    ...
                    string NoteDate = d.LSDate == new DateTime(1970, 1, 1) ? "-" : d.LSDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

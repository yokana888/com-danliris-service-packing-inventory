        ...
        public List<AnnualOmzetByUnitViewModel> GetData(int year, int offset)
        {
            ...
            var Query1 = (from a in queryInv
                          ...
                          select new GarmentDetailOmzetByUnitReportViewModel
                          {
                              ...
                              Month = MONTHS[c.TruckingDate.AddHours(_identityProvider.TimezoneOffset).Month - 1],
                              MonthName = MONTH_NAMES[c.TruckingDate.AddHours(_identityProvider.TimezoneOffset).Month - 1],
                              RONumber = b.RONo,
                              UnitCode = b.UnitCode,
                              UnitName = "",
                              CurrencyCode = "USD",
            ...
            //             }).Distinct().ToList();
            //
            var currencyFilters = Query1
                                   .GroupBy(o => new { o.PEBDate, o.CurrencyCode })
                                   .Select(o => new CurrencyFilter { date = o.Key.PEBDate.AddHours(_identityProvider.TimezoneOffset).DateTime, code = o.Key.CurrencyCode })
                                   .ToList();
            ...
            {
                rate = Convert.ToDecimal(currencies.Where(q => q.code == data.CurrencyCode && q.date == data.PEBDate.AddHours(_identityProvider.TimezoneOffset).DateTime).Select(s => s.rate).LastOrDefault());

                data.Rate = rate;
                data.AmountIDR = rate * data.Amount;
            }
            //
            var QueryOmzet1 = (from a in Query1.Distinct()
            ...
            //
            var QueryOmzet2 = (from a in Query1.Distinct()
            ...
            //
            var QueryOmzet3 = (from a in Query1.Distinct()
            ...
            //
            var QueryOmzet4 = (from a in Query1.Distinct()
            ...
            //
            var QueryOmzet5 = (from a in Query1.Distinct()

    ...
    public class GarmentShipmentMonitoringService : IGarmentShipmentMonitoringService
    {
        ...
        public IQueryable<GarmentShipmentMonitoringViewModel> GetData(string buyerAgent, string invoiceNo, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            var NewQ1 = (from a in queryInv
                         ...
                         where a.IsDeleted == false && c.IsDeleted == false && (InvAdj == null || InvAdj.IsDeleted == false)
            ...
            var NewQ3 = (from aa in NewQ2
                         ...
                         where bb.IsDeleted == false && (CL == null || CL.IsDeleted == false) && (CA == null || CA.IsDeleted == false)
            ...
            else
            {
                ...
                foreach (KeyValuePair<string, List<GarmentShipmentMonitoringViewModel>> BuyerBrand in dataByBrand)
                {
                    ...
                    foreach (GarmentShipmentMonitoringViewModel item in BuyerBrand.Value)
                    {
                        ...
                        string InvDate = item.InvoiceDate == new DateTime(1970, 1, 1) ? "-" : item.InvoiceDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string SailDate = item.SailingDate == new DateTime(1970, 1, 1) ? "-" : item.SailingDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string PEBDate = item.PEBDate == DateTimeOffset.MinValue ? "-" : item.PEBDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string TruckDate = item.TruckingDate == DateTimeOffset.MinValue ? "-" : item.TruckingDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string BookDate = item.BookingDate == new DateTime(1970, 1, 1) ? "-" : item.BookingDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string ExFtyDate = item.ExpFactoryDate == new DateTime(1970, 1, 1) ? "-" : item.ExpFactoryDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

                        string DocDate = item.DocSendDate == new DateTime(1970, 1, 1) ? "-" : item.DocSendDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string DueDate = item.DueDate == new DateTime(1970, 1, 1) ? "-" : item.DueDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string PayDate = item.PaymentDate == new DateTime(1970, 1, 1) ? "-" : item.PaymentDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

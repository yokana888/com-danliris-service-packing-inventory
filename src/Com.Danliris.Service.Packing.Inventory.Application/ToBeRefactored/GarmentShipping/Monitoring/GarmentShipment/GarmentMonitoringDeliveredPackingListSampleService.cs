    ...
    public class GarmentMonitoringDeliveredPackingListSampleService : IGarmentMonitoringDeliveredPackingListSample
    {
        ...
        public MemoryStream GenerateExcel(string invoiceNo, string paymentTerm, DateTimeOffset? dateFrom, DateTimeOffset? dateTo, int offset)
        {
            ...
            //masuk sini
            {
                ...
                {
                    string InvDate = d.Date == new DateTime(1970, 1, 1) ? "-" : d.Date.AddHours(offset).ToString("dd MMM yyyy", new CultureInfo("id-ID"));
                    string TruckDate = d.TruckingDate == DateTimeOffset.MinValue ? "-" : d.TruckingDate.AddHours(offset).ToString("dd MMM yyyy", new CultureInfo("id-ID"));

            ...
            else
            {
                ...
                foreach (KeyValuePair<string, List<GarmentCreditAdviceMonitoringViewModel>> BuyerBrand in dataByBrand)
                {
                    ...
                    foreach (GarmentCreditAdviceMonitoringViewModel item in BuyerBrand.Value)
                    {
                        ...
                        string DocDate = item.DocUploadDate == new DateTime(1970, 1, 1) ? "-" : item.DocUploadDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string SRDate = item.SRDate == new DateTime(1970, 1, 1) ? "-" : item.SRDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

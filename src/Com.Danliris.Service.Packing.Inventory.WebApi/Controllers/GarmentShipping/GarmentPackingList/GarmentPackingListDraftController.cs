...
namespace Com.Danliris.Service.Packing.Inventory.WebApi.V8.Controllers.GarmentShipping.GarmentPackingList
{
    ...
    public class GarmentPackingListDraftController : ControllerBase
    {
        ...
        [HttpGet("{id}/carton/xls")]
        public async Task<IActionResult> GetExcelFilterCarton([FromRoute] int id)
        {
            try
            {
                VerifyUser();
                var result = await _service.ReadExcelByIdFilterCarton(id);
        ...
        [HttpGet("{id}/md/carton")]
        public async Task<IActionResult> GetPdfFilterCartonMD([FromRoute] int id)
        {
            try
            {
                VerifyUser();
                var result = await _service.ReadPdfFilterCartonMD(id);

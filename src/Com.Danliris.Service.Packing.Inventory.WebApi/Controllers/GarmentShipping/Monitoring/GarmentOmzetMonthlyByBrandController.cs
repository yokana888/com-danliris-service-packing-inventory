...
namespace Com.Danliris.Service.Packing.Inventory.WebApi.V8.Controllers.GarmentShipping.Monitoring
{
    ...
    [Route("v1/garment-shipping/monitoring/garment-omzet-by-buyer-brand")]
    [Authorize]
    public class GarmenOmzetMonthlyByBrandController : ControllerBase
    {
        private string ApiVersion = "1.0.0";
        private readonly IGarmentOmzetMonthlyByBrandService _service;
        private readonly IIdentityProvider _identityProvider;

        public GarmenOmzetMonthlyByBrandController(IGarmentOmzetMonthlyByBrandService service, IIdentityProvider identityProvider)
        {
            _service = service;
        ...
        public IActionResult GetXls(string buyerAgent, DateTime? dateFrom, DateTime? dateTo)
        {
            try
            {
                ...
                string filename = string.Format("Garment Omzet Per Buyer Brand - {0}.xlsx", DateTime.UtcNow.ToString("ddMMyyyy"));

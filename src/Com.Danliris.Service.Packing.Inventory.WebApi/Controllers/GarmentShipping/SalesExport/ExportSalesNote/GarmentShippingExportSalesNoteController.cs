...
namespace Com.Danliris.Service.Packing.Inventory.WebApi.V8.Controllers.GarmentShipping.SalesExport.ExportSalesNote
{
    [Produces("application/json")]
    [Route("v1/garment-shipping/export-sales-notes")]
    [Authorize]
    public class GarmentShippingExportSalesNoteController : ControllerBase
    {
        private readonly IGarmentShippingExportSalesNoteService _service;
        private readonly IGarmentExportCoverLetterService _clservice;
        private readonly IIdentityProvider _identityProvider;
        private readonly IValidateService _validateService;

        public GarmentShippingExportSalesNoteController(IGarmentShippingExportSalesNoteService service, IGarmentExportCoverLetterService clservice, IIdentityProvider identityProvider, IValidateService validateService)
        {
            _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] GarmentShippingExportSalesNoteViewModel viewModel)
        {
            try
            {
                VerifyUser();
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromRoute] int id, [FromBody] GarmentShippingExportSalesNoteViewModel viewModel)
        {
            try
            {
                VerifyUser();
        ...
        public async Task<IActionResult> PutRejectShipping([FromRoute] int id, [FromBody] GarmentShippingExportSalesNoteViewModel viewModel)
        {
            try
            {
                VerifyUser();
        ...
        public async Task<IActionResult> PutRejectFinance([FromRoute] int id, [FromBody] GarmentShippingExportSalesNoteViewModel viewModel)
        {
            try
            {
                VerifyUser();
        ...
        public async Task<IActionResult> GetPDF([FromRoute] int Id)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    GarmentExportCoverLetterViewModel cl = await _clservice.ReadByExportSalesNoteId(model.Id);
                    var PdfTemplate = new GarmentShippingExportSalesNotePdfTemplate();
        ...
        public IActionResult GetExportSalesDebtorNow(int month, int year)
        {
            try
            {
                var data = _service.ReadShippingExportSalesNoteListNow(month, year);
        ...
        public IActionResult GetExportSalesDebtor([FromQuery] string type, [FromQuery] int month, [FromQuery] int year)
        {
            try
            {
                var data = _service.ReadExportSalesDebtor(type, month, year);
                return Ok(new
                {
                    data
                });

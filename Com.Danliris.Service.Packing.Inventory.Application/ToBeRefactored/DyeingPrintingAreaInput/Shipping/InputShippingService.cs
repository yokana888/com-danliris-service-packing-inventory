...
namespace Com.Danliris.Service.Packing.Inventory.Application.ToBeRefactored.DyeingPrintingAreaInput.Shipping
{
    public class InputShippingService : IInputShippingService
    {
        ...
        private InputShippingViewModel MapToViewModelBon(DyeingPrintingAreaInputModel model)
        {
            var vm = new InputShippingViewModel()
            {
                Active = model.Active,
                ...
                Shift = model.Shift,
                ShippingType = model.ShippingType,
                ShippingProductionOrders = model.DyeingPrintingAreaInputProductionOrders.GroupBy( r => new { r.ProductionOrderNo , r.Grade}).Select(s => new InputShippingProductionOrderViewModel()
                {
                    ...
                    QtyPacking = s.Sum( d => d.PackagingQty),
                    InputQtyPacking = s.Sum( d => d.InputPackagingQty),
                    PackingType = s.First().PackagingType,
                    Qty = s.Sum( d => d.Balance),
            ...
            if (model == null)
            {
                ...
                model = new DyeingPrintingAreaInputModel(viewModel.Date, viewModel.Area, viewModel.Shift, bonNo, viewModel.Group, viewModel.ShippingType, viewModel.ShippingProductionOrders.Select(s =>
                     new DyeingPrintingAreaInputProductionOrderModel(viewModel.Area, s.DeliveryOrder.Id, s.DeliveryOrder.No, s.ProductionOrder.Id, s.ProductionOrder.No, s.ProductionOrder.Type,
                         ...
                         s.ProductPackingCode, s.HasPrintingProductPacking, s.PackingLength, s.InputQuantity, s.InputQtyPacking, s.DeliveryOrderRetur.Id, s.DeliveryOrderRetur.No, s.FinishWidth, viewModel.Date, s.DeliveryOrder.Name, s.InventoryType, s.MaterialOrigin, s.DeliveryOrderSalesType,
                ...
                foreach (var item in model.DyeingPrintingAreaInputProductionOrders)
                {
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.IN, model.Id, model.BonNo, item.ProductionOrderId, item.ProductionOrderNo,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.InputQuantity, item.Id, item.ProductionOrderType,  item.ProductTextileId, item.ProductTextileCode, item.ProductTextileName, item.Grade, 
                        null, item.PackagingType, item.InputPackagingQty, item.PackagingUnit, item.PackagingLength, item.InventoryType);
            ...
            else
            {
                foreach (var item in viewModel.ShippingProductionOrders)
                {
                    ...
                    var modelItem = new DyeingPrintingAreaInputProductionOrderModel(viewModel.Area, item.DeliveryOrder.Id, item.DeliveryOrder.No, item.ProductionOrder.Id,
                        ...
                        item.InputQtyPacking, item.DeliveryOrderRetur.Id, item.DeliveryOrderRetur.No, item.FinishWidth, viewModel.Date, item.DeliveryOrder.Name, item.InventoryType, item.MaterialOrigin, item.DeliveryOrderSalesType,
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.IN, model.Id, model.BonNo, item.ProductionOrder.Id, item.ProductionOrder.No,
                        ...
                        item.Grade, null, item.PackingType, item.InputQtyPacking, item.Packing, item.PackingLength, item.InventoryType);
                ...
                if (model == null)
                {
                    ...
                    model = new DyeingPrintingAreaInputModel(viewModel.Date, item.Key, viewModel.Shift, bonNo, viewModel.Group,
                                    item.Select(s => new DyeingPrintingAreaInputProductionOrderModel(item.Key, s.ProductionOrder.Id,
                                        ...
                                        s.HasPrintingProductSKU, s.ProductPackingId, s.FabricPackingId, s.ProductPackingCode, s.HasPrintingProductPacking, s.PackingLength, s.InputQuantity, s.InputQtyPacking, s.FinishWidth, s.DateIn, s.InventoryType, s.MaterialOrigin, s.ProductPackingCode, 
                                        s.ProductTextile.Id, s.ProductTextile.Code, s.ProductTextile.Name)).ToList());
                    ...
                    foreach (var detail in item)
                    {
                        ...
                        var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, detail.MaterialOrigin, item.Key, DyeingPrintingArea.IN, model.Id, model.BonNo, detail.ProductionOrder.Id, detail.ProductionOrder.No,
                            detail.CartNo, detail.Buyer, detail.Construction, detail.Unit, detail.Color, detail.Motif, detail.UomUnit, detail.InputQuantity, itemModel.Id, detail.ProductionOrder.Type, detail.ProductTextile.Id, detail.ProductTextile.Code, detail.ProductTextile.Name,
                            detail.Grade, null, detail.PackingType, detail.InputQtyPacking, detail.Packing, detail.PackingLength, detail.InventoryType);
                ...
                else
                {
                    foreach (var detail in item)
                    {
                        ...
                        var modelItem = new DyeingPrintingAreaInputProductionOrderModel(item.Key, detail.ProductionOrder.Id, detail.ProductionOrder.No, detail.ProductionOrder.Type,
                            ...
                            detail.InputQuantity, detail.InputQtyPacking, detail.FinishWidth, detail.DateIn, detail.InventoryType, detail.MaterialOrigin, detail.ProductPackingCode, detail.ProductTextile.Id, detail.ProductTextile.Code, detail.ProductTextile.Name);
                        ...
                        var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, detail.MaterialOrigin, item.Key, DyeingPrintingArea.IN, model.Id, model.BonNo, detail.ProductionOrder.Id, detail.ProductionOrder.No,
                            detail.CartNo, detail.Buyer, detail.Construction, detail.Unit, detail.Color, detail.Motif, detail.UomUnit, detail.InputQuantity, modelItem.Id, detail.ProductionOrder.Type, detail.ProductTextile.Id, detail.ProductTextile.Code, detail.ProductTextile.Name,
                            detail.Grade, null, detail.PackingType, detail.InputQtyPacking, detail.Packing, detail.PackingLength, detail.InventoryType);
        ...
        public async Task<int> Update(int id, InputShippingViewModel viewModel)
        {
            ...
            var model = new DyeingPrintingAreaInputModel(viewModel.Date, viewModel.Area, viewModel.Shift, viewModel.BonNo, viewModel.Group, viewModel.ShippingType, viewModel.ShippingProductionOrders.Select(s =>
                     new DyeingPrintingAreaInputProductionOrderModel(viewModel.Area, s.DeliveryOrder.Id, s.DeliveryOrder.No, s.ProductionOrder.Id, s.ProductionOrder.No, s.ProductionOrder.Type,
                         ...
                         s.ProductPackingId, s.FabricPackingId, s.ProductPackingCode, s.HasPrintingProductPacking, s.PackingLength, s.InputQuantity, s.InputQtyPacking, s.DeliveryOrderRetur.Id, s.DeliveryOrderRetur.No, s.FinishWidth, s.DateIn, s.DeliveryOrder.Name, s.InventoryType, s.MaterialOrigin, s.DeliveryOrderSalesType,
        ...
        public MemoryStream GenerateExcel(DateTimeOffset? dateFrom, DateTimeOffset? dateTo, string type, int offSet)
        {
            ...
            {
                query = query.Where(s => dateFrom.Value.Date <= s.Date.AddHours(offSet).Date &&
                            s.Date.AddHours(offSet).Date <= dateTo.Value.Date);
            }
            else if (!dateFrom.HasValue && dateTo.HasValue)
            {
                query = query.Where(s => s.Date.AddHours(offSet).Date <= dateTo.Value.Date);
            }
            else if (dateFrom.HasValue && !dateTo.HasValue)
            {
                query = query.Where(s => dateFrom.Value.Date <= s.Date.AddHours(offSet).Date);

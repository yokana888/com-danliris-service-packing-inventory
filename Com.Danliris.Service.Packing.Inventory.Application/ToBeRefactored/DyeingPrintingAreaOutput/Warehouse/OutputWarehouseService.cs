    ...
    public class OutputWarehouseService : IOutputWarehouseService
    {
        ...
        private async Task<int> CreateOut(OutputWarehouseViewModel viewModel)
        {
            ...
            if (model == null)
            {
                ...
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.OUT, model.Id, model.BonNo, item.ProductionOrder.Id, item.ProductionOrder.No,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.ProductPackingCode.Split(",").Count() * item.Quantity, item.Id, item.ProductionOrder.Type, item.ProductTextile.Id, item.ProductTextile.Code, item.ProductTextile.Name, item.Grade, null,
            ...
            else
            {
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    ...
                    var modelItem = new DyeingPrintingAreaOutputProductionOrderModel(item.ProductionOrder.Id,
                        ...
                        item.DestinationBuyerName, item.InventoryType, item.MaterialOrigin, item.DeliveryOrderSalesType, item.ProductTextile.Id, item.ProductTextile.Code, item.ProductTextile.Name);
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.OUT, model.Id, model.BonNo, item.ProductionOrder.Id, item.ProductionOrder.No,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, GetBalance(item), item.Id, item.ProductionOrder.Type, item.ProductTextile.Id, item.ProductTextile.Code, item.ProductTextile.Name, item.Grade, null,
        ...
        private async Task<int> CreateAdj(OutputWarehouseViewModel viewModel)
        {
            ...
            if (model == null)
            {
                model = new DyeingPrintingAreaOutputModel(viewModel.Date, viewModel.Area, viewModel.Shift, bonNo, true, "", viewModel.Group, type,
                        viewModel.WarehousesProductionOrders.Select(s => new DyeingPrintingAreaOutputProductionOrderModel(s.ProductionOrder.Id, s.ProductionOrder.No,
                            ...
                            s.HasPrintingProductSKU, s.ProductPackingId, s.FabricPackingId, s.ProductPackingCode, s.HasPrintingProductPacking, s.Quantity, s.FinishWidth, s.DateIn, viewModel.Date, s.DestinationBuyerName, s.InventoryType, s.MaterialOrigin, s.DeliveryOrderSalesType,
                ...
                foreach (var item in model.DyeingPrintingAreaOutputProductionOrders)
                {
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, type, model.Id, model.BonNo, item.ProductionOrderId, item.ProductionOrderNo,
                            item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.Balance, item.Id, item.ProductionOrderType, item.ProductTextileId, item.ProductTextileCode, item.ProductTextileName, item.Grade, null,
            ...
            else
            {
                foreach (var item in viewModel.WarehousesProductionOrders)
                {
                    ...
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.OUT, model.Id, model.BonNo, item.ProductionOrder.Id, item.ProductionOrder.No,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.Balance, modelItem.Id, item.ProductionOrder.Type, item.ProductTextile.Id, item.ProductTextile.Code, item.ProductTextile.Name, item.Grade, null,
        ...
        private MemoryStream GenerateExcelOut(DyeingPrintingAreaOutputModel model, int offSet)
        {
            ...
            else
            {
                ...
                {
                    var dataIn = item.DateIn.Equals(DateTimeOffset.MinValue) ? "" : item.DateIn.AddHours(offSet).Date.ToString("d");
                    var dataOut = item.DateIn.Equals(DateTimeOffset.MinValue) ? "" : item.DateOut.AddHours(offSet).Date.ToString("d");
            ...
            foreach (var item in model.DyeingPrintingAreaOutputProductionOrders)
            {
                if (!item.HasNextAreaDocument)
                {
                    var movementModel = new DyeingPrintingAreaMovementModel(model.Date, item.MaterialOrigin, model.Area, DyeingPrintingArea.OUT, model.Id, model.BonNo, item.ProductionOrderId, item.ProductionOrderNo,
                       item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, item.Balance * -1, item.Id, item.ProductionOrderType, item.ProductTextileId, item.ProductTextileCode, item.ProductTextileName, item.Grade, null,
        ...
        public MemoryStream GenerateExcelAll(DateTimeOffset? dateFrom, DateTimeOffset? dateTo, string type, int offSet)
        {
            ...
            {
                warehouseData = warehouseData.Where(s => dateFrom.Value.Date <= s.Date.AddHours(offSet).Date &&
                            s.Date.AddHours(offSet).Date <= dateTo.Value.Date);
            }
            else if (!dateFrom.HasValue && dateTo.HasValue)
            {
                warehouseData = warehouseData.Where(s => s.Date.AddHours(offSet).Date <= dateTo.Value.Date);
            }
            else if (dateFrom.HasValue && !dateTo.HasValue)
            {
                warehouseData = warehouseData.Where(s => dateFrom.Value.Date <= s.Date.AddHours(offSet).Date);
        ...
        public MemoryStream GenerateExcelAllBarcode(DateTimeOffset? dateFrom, DateTimeOffset? dateTo, int offSet)
        {
            ...
            foreach (var data in modelAll)
            {
                foreach (var item in data.SppList) {

                    foreach (var packingCode in item.ProductPackingCodes)
                    {
                        var barcodeInfo = new BarcodeInfoViewModel2()
                        {
                            ...
                            PackingLength = item.PackingLength,
                            PackagingQty = 1,
                            Grade = item.Grade,
                        };
            ...
            foreach (var item in dbModel.DyeingPrintingAreaOutputProductionOrders.Where(d => !d.HasNextAreaDocument && !d.IsDeleted))
            {
                ...
                if (newQtyPacking != 0 && newBalance != 0)
                {
                    var movementModel = new DyeingPrintingAreaMovementModel(viewModel.Date, item.MaterialOrigin, viewModel.Area, DyeingPrintingArea.OUT, model.Id, model.BonNo, item.ProductionOrderId, item.ProductionOrderNo,
                        item.CartNo, item.Buyer, item.Construction, item.Unit, item.Color, item.Motif, item.UomUnit, newBalance, item.Id, item.ProductionOrderType, item.ProductTextileId, item.ProductTextileCode, item.ProductTextileName, item.Grade, null,

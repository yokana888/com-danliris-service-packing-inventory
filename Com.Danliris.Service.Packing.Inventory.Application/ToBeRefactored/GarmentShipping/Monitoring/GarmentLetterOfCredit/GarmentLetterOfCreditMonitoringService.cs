        ...
        public MemoryStream GenerateExcel(string buyerAgent, string lcNo, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            else
            {
                ...
                foreach (var d in Query)
                {
                    ...
                    string LCDate = d.LCDate == new DateTime(1970, 1, 1) ? "-" : d.LCDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string EDate = d.ExpiredDate == new DateTime(1970, 1, 1) ? "-" : d.ExpiredDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string ShipDate = d.LatestShipment == DateTimeOffset.MinValue ? "-" : d.LatestShipment.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

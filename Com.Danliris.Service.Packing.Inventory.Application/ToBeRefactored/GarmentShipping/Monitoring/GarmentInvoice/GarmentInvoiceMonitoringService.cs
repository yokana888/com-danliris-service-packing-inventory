    ...
    public class GarmentInvoiceMonitoringService : IGarmentInvoiceMonitoringService
    {
        ...
        public IQueryable<GarmentInvoiceMonitoringViewModel> GetData(string buyerAgent, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            var newQ = (from a in queryInv
                        ...
                        where a.IsDeleted == false && b.IsDeleted == false && (CA == null || CA.IsDeleted == false)
        ...
        public MemoryStream GenerateExcel(string buyerAgent, DateTime? dateFrom, DateTime? dateTo, int offset)
        {
            ...
            else
            {
                ...
                foreach (var d in Query)
                {
                    ...
                    string InvDate = d.InvoiceDate == new DateTime(1970, 1, 1) ? "-" : d.InvoiceDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string SailDate = d.SailingDate == new DateTime(1970, 1, 1) ? "-" : d.SailingDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string PEBDate = d.PEBDate == DateTimeOffset.MinValue ? "-" : d.PEBDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string TruckDate = d.TruckingDate == DateTimeOffset.MinValue ? "-" : d.TruckingDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string CADate = d.CADate == new DateTime(1970, 1, 1) ? "-" : d.CADate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                    string PayDate = d.PaymentDate == new DateTime(1970, 1, 1) ? "-" : d.PaymentDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));

        ...
        public MemoryStream GenerateExcel(DateTime? dateFrom, DateTime? dateTo, int offset)
       {
            ...
            else
            {
                ...
                foreach (KeyValuePair<string, List<GarmentLocalSalesBookViewModel>> SupplName in dataBySupplier)
                {
                    ...
                    {
                        index++;
                        string LSDate = data.LSDate == new DateTime(1970, 1, 1) ? "-" : data.LSDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
                        string CLDate = data.CLDate == new DateTime(1970, 1, 1) ? "-" : data.CLDate.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("us-US"));
